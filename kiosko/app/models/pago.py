...
class Pago:
    ...
    def crear(self):
        execute_query(
            "INSERT INTO pagos (pedido_id, monto, metodo, estado) VALUES (%s,%s,%s,%s)",
            (self.pedido_id, self.monto, self.metodo, self.estado), commit=True)

    @staticmethod
    def procesar_pago_simple(pedido_id, monto):
        pago=Pago(pedido_id=pedido_id, monto=monto)
        pago.crear()
        return pago
