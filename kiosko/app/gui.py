...
class KioscoGUI:
    ...
    def init_ui(self):
        ...
        tk.Button(btn_frame, text="Agregar al carrito", command=self.add_to_cart).grid(row=0, column=0, padx=4)
        tk.Button(btn_frame, text="Quitar selección", command=self.remove_selected).grid(row=0, column=1, padx=4)
        tk.Button(btn_frame, text="Ver pedidos", command=self.ver_pedidos).grid(row=0, column=2, padx=4)
    ...
    def pagar(self):
        ...
        cliente = Cliente(nombre=nombre_cliente, telefono=None)
        cliente_id = cliente.crear()  # <-- AQUÍ SE OBTIENE EL ID REAL

        # Preparar productos para pedido
        items = [{'id': it['id'], 'precio': it['precio'], 'cantidad': it['cantidad']} for it in self.carrito]
